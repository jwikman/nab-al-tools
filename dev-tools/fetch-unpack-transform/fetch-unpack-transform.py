...
xlf_file_re = re.compile(r".*Base Application\.[a-z]{2}-[A-Z]{2}\.xlf", re.MULTILINE | re.IGNORECASE)
language_source_zip_re = re.compile(r"Applications\\BaseApp\\Source\\.* language \(.*\).Source.zip", re.MULTILINE | re.IGNORECASE)

artifacts = [ "https://bcartifacts.azureedge.net/onprem/17.1.18256.18474/se" ]  # Update when needed
...
def parse_translations():
    ...
    for f in xlf_files:
        ...
        for node in root:
            ...
            trans_unit_list = [trans_unit for trans_unit in node[0][0] if valid_tag(trans_unit)]

        for i, trans_unit in enumerate(trans_unit_list):
            source = trans_unit[0].text
            target = trans_unit[1].text

            if source in translations:
                if not target in translations[source]:
