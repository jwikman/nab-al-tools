...
export async function generateExternalDocumentation(
  ...
  if (settings.createTocFilesForDocs) {
    ...
    saveContentToFile(settings, appManifest, tocPath, tocContent);
    ...
    obsoleteControls.forEach((control) => {
      ...
      if (obsoleteInfo) {
        tableContent += `| ${objText} | ${controlTypeToText(control)} | ${
          control.name
        } | ${obsoleteInfo.obsoleteReason} | ${obsoleteInfo.obsoleteTag} |\n`;
      }
    });
    tableContent += "\n";
...
function generateObjectDocumentation(
  ...
  if (
    ...
    objectIndexContent += getTableFieldsTable(publicObjects, object);
  }
  ...
  if (
    ...
    objectIndexContent += getApiPageFieldsTable(
      settings,
      publicObjects,
      object
    );
...
function generateProcedurePages(
  ...
  if (createTocSetting) {
    ...
    saveContentToFile(settings, appManifest, tocFilepath, tocContent);
  }
}

function getTableFieldsTable(
  publicObjects: ALObject[],
  object: ALObject
): string {
  let objectIndexContent = "";
  const fields = (object.controls.filter(
    ...
    (fld) => !fld.isObsoletePending() && !fld.isObsolete() && !fld.isSystemField
  );
  if (fields.length > 0) {
    const printSummary =
...
function getEnumValuesTable(object: ALObject): string {
  ...
  ) as ALEnumValue[]).filter((o) => !o.isObsoletePending() && !o.isObsolete());
  if (values.length > 0) {
