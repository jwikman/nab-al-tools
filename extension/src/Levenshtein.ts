...
export function matchStringAgainstListOfStrings(
  source: string,
  targets: string[],
  threshold = 0.6
): Levenshtein[] {
  ...
  for (const t of targets) {
    const match = getLevenshteinRatioAndDistance(
      source,
      t,
      LevenshteinCost.ratio
    );
    match.threshold = threshold;
    if (match.ratio !== undefined) {
      if (match.ratio >= threshold) {
        match.distance = getLevenshteinRatioAndDistance(
          source,
          t,
          LevenshteinCost.distance
        ).distance;
        result.push(match);
      }
    }
...
export function getLevenshteinRatioAndDistance(
  source: string,
  target: string,
  calcCost: LevenshteinCost
): Levenshtein {
  let ratio = 0;
  ...
  if (calcCost === LevenshteinCost.ratio) {
    ratio =
      (source.length + target.length - distance[source.length][target.length]) /
      (source.length + target.length);
    return { ratio: ratio, source: source, target: target };
  }

  return {
    distance: distance[source.length][target.length],
    source: source,
    target: target,
  };
}
