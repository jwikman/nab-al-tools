...
export class Settings {
  ...
  private static _getConfigSettings(resourceUri?: vscode.Uri): void {
    this.config = vscode.workspace.getConfiguration(WORKSPACEKEY, getWorkspaceFolder(resourceUri).uri);
  ...
  private static _getAppSettings(resourceUri?: vscode.Uri): void {
    const appSettingsFolder: string = getWorkspaceFolder(resourceUri).uri.fsPath;
    const appSettings = JSON.parse(fs.readFileSync(join(appSettingsFolder, "app.json"), "utf8"));
  ...
  private static _getLaunchSettings(resourceUri?: vscode.Uri): void {
    const vscodeSettingsFolder: string = join(getWorkspaceFolder(resourceUri).uri.fsPath, ".vscode");

    const launchSettings = JSON.parse(fs.readFileSync(join(vscodeSettingsFolder, "launch.json"), "utf8"));
