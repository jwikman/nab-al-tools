...
import { ALObject2 } from '../ALObject/ALObject2';
...
suite("Classes.AL Functions Tests", function () {
    ...
    test("Valid Object Desriptors", function () {
        ...
        for (let index = 0; index < objectDescriptorArr.length; index++) {
            ...
            let obj = ALObject2.getALObject(item.ObjectDescriptor, false);
            if (!obj) {
    ...
    test("Invalid Object Desriptors", function () {
        ...
        for (let index = 0; index < objectDescriptorArr.length; index++) {
            ...
            let obj = null;
            try {
                obj = ALObject2.getALObject(item, false);
            } catch (error) {
    ...
    test("AL Codeunit procedure(param with parenthesis) Xliff", function () {
        let alObj = ALObject2.getALObject(ALObjectTestLibrary.getCodeunitWithFunctionsWithParenthesisParam(), true);
        if (!alObj) {
    ...
    test("AL Page with groups and repeater Xliff", function () {
        let alObj = ALObject2.getALObject(ALObjectTestLibrary.getPageWithGroupsAndRepeater(), true);
        if (!alObj) {
            assert.fail('Could not find object');
        }
        let mlObjects = alObj.getMultiLanguageObjects(true);
    ...
    test("AL Table Xliff", function () {
        let alObj = ALObject2.getALObject(ALObjectTestLibrary.getTable(), true);
        if (!alObj) {
            assert.fail('Could not find object');
        }
        let mlObjects = alObj.getMultiLanguageObjects(true);
    ...
    test("AL RoleCenterPage Xliff", function () {
        let alObj = ALObject2.getALObject(ALObjectTestLibrary.getRoleCenterPage(), true);
        if (!alObj) {
            assert.fail('Could not find object');
        }
        let mlObjects = alObj.getMultiLanguageObjects(true);
