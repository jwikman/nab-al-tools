...
- Changed:
  ...
  - **Glossary output format**: The `getGlossaryTerms` tool now outputs TSV (tab-separated values) format by default instead of JSON. TSV columns: `source`, `target`, `description`. Use the new `outputFormat: "json"` parameter to request JSON format when needed. This change applies to both the Language Model Tool and MCP server.
