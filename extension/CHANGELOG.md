...
- Added:
  - Added `sampling` parameter to `getTranslatedTextsMap` tool. When set to `"even"`, selects evenly-spaced entries across the full translation map instead of returning the first N entries. This provides better vocabulary coverage when loading reference translations for AI-assisted translation workflows. When `sampling="even"`, the `offset` parameter is ignored.
  - Added `buildAlPackage` Language Model Tool that builds AL projects and returns comprehensive compilation diagnostics (errors and warnings) with source code context. This tool addresses the limitation of the `al_build` tool which only returns generic "Build failed" messages without specific error details. The `buildAlPackage` tool provides: file paths with diagnostics, line and column numbers (1-based), diagnostic severity (Error/Warning), diagnostic codes and messages, source code context (5 lines before/after each diagnostic with the diagnostic line marked with `>>>`), distinction between main app and dependency diagnostics via `isMainApp` field, and support for both `.al` files and `app.json` diagnostics. The tool automatically opens and focuses the specified `app.json` file before building (required by AL extension), then executes the `al.package` command and collects all compilation diagnostics. This detailed diagnostic information enables LLMs to diagnose and fix compilation issues effectively, making it particularly valuable for translation workflows where build validation is critical. Note: This tool may be deprecated once the `al_build` tool is improved to return proper diagnostic information.
  ...
  - Added **NAB-XLF-Translator agent** for BC AL XLF translation with three workflow modes: Translation (`translateXlfFiles` prompt), Review (`reviewXlfFiles` prompt with batch approval), and Glossary Management (`manageGlossary` prompt for creating/managing project-specific glossary.tsv files). Each mode uses dedicated instruction files (`translation-workflow.instructions.md`, `review-translation-workflow.instructions.md`, `glossary-management.instructions.md`) and reloads context on switch. Local glossary terms override built-in BC glossary. Includes `writing-instructions.instructions.md` with guidelines for creating effective AI instruction files.
  ...
  - Added new setting `NAB.UseTargetStates` to replace `NAB.UseExternalTranslationTool` with a clearer name. When enabled, the extension uses XLIFF target state attributes (like 'translated', 'needs-translation', etc.) instead of [NAB: *] tags. This is useful when working with external translation tools, Copilot-assisted translations, or any workflow that benefits from XLIFF state management.
  - Added `outputFormat` parameter to Language Model Tools (ChatTools only, not MCP). Supports `"json"` and `"tsv"` output formats. Glossary defaults to TSV; translation tools default to JSON. TSV is not supported for tools with nested array fields (`getTranslatedTextsByState`, `getTextsByKeyword`). The `getTranslatedTextsMap` tool supports TSV by flattening: each source-target pair becomes a separate row (multiple translations for the same source text produce multiple rows).
- Changed:
  - **Deprecated Setting**: `NAB.UseExternalTranslationTool` has been renamed to `NAB.UseTargetStates` for clarity. The old setting will continue to work for backward compatibility, but users should update their settings to use `NAB.UseTargetStates`. The deprecated setting will be removed in a future major version. All related setting descriptions now reference `UseTargetStates` instead of the old name.
  - **XLIFF Editor**: Enhanced checkbox behavior when `NAB.UseTargetStates` is enabled. The checkbox now properly advances translation states (translated → signed-off → final) based on the current filter, and the checkbox header label dynamically changes to reflect the next state. Added a new filter option "Show state 'final'" that displays only translation units with the "final" state, where all items are checked by default and unchecking reverts them to "signed-off" state.
  ...
  - **Glossary output format**: The `getGlossaryTerms` Language Model Tool (ChatTools) now outputs TSV (tab-separated values) format by default instead of JSON. TSV columns: `source`, `target`, `description`. Use the new `outputFormat: "json"` parameter to request JSON format when needed. The MCP server version outputs JSON by default.
  - **MCP compact JSON serialization**: `compactJsonSerialize` now formats envelope objects (containing `texts` or `items` arrays) with one item per line in the inner array, matching the documented behavior. Previously, envelope objects were serialized as single-line minified JSON.
  - **Translation tool output envelope**: The `getTextsToTranslate`, `getTranslatedTextsMap`, `getTranslatedTextsByState`, and `getTextsByKeyword` tools now wrap output in an envelope object with `sourceLanguage` at the top level. Per-item `sourceLanguage` is removed from serialized output. For `getTextsToTranslate`: `{ sourceLanguage, totalUntranslatedCount, returnedCount, texts: [...] }`. For other tools: `{ sourceLanguage, items: [...] }`.
  - **MCP server compact JSON output**: All MCP server tool responses now use compact JSON serialization (one item per line) instead of pretty-printed JSON, reducing response size.
  - **Translation workflow restructured for subagent architecture**: The NAB-XLF-Translator agent now supports self-looping translation with fresh context per chunk. Each translator subagent fetches glossary and translated texts map directly at session start, then loops `getTextsToTranslate → translate → setTranslations` until complete.
  - **Instruction files condensed**: Agent and instruction files reduced by ~33% in total character count (~28K characters saved) without information loss, improving token efficiency.
  - Note: The glossary TSV and sourceLanguage envelope changes are breaking changes. The `outputFormat` parameter provides backward compatibility — consumers can request `outputFormat: "json"` to get the previous JSON format for glossary.
- Fixes:
  - Fixed an issue where the `NAB: Refresh XLF files from g.xlf` command failed to find g.xlf files with special characters like square brackets `[` and `]` or curly braces `{` and `}` in the filename or directory path (e.g., `App Name [CORP_STD].g.xlf` or directory `D:\Projects\App [TEST]\Translations`). Thanks to [@Urmas-Pill](https://github.com/Urmas-Pill) for reporting this in [issue 573](https://github.com/jwikman/nab-al-tools/issues/573).
  - Fixed an issue in the XLIFF Editor where changing filters would cause an `ApplicationManifestError` when the active workspace folder could not be determined.
...
- Changed:
  ...
  - **Enhanced Translation Propagation**: The `saveTranslatedTexts` tool (both Language Model Tool and MCP Server) now automatically propagates translations to other translation units with matching source text:
    ...
    - For External mode (when `useTargetStates` is enabled): Only propagates to units with state `needsTranslation`, `new`, or empty
