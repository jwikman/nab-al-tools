...
- Fixes:
  - Fixed an issue where the `NAB: Refresh XLF files from g.xlf` command failed to find g.xlf files with square brackets `[` and `]` or curly braces `{` and `}` in the filename (e.g., `App Name [CORP_STD].g.xlf`). Thanks to [@Urmas-Pill](https://github.com/Urmas-Pill) for reporting this in [issue 573](https://github.com/jwikman/nab-al-tools/issues/573).
