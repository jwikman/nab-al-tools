...
- Added:
  - Added `buildAlPackage` Language Model Tool that builds AL projects and returns comprehensive error diagnostics with source code context. This tool addresses the limitation of the `al_build` tool which only returns generic "Build failed" messages without specific error details. The `buildAlPackage` tool provides: file paths with errors, line and column numbers (1-based), error codes and messages, source code context (5 lines before/after each error with the error line marked with `>>>`), distinction between main app and dependency errors via `isMainApp` field, and support for both `.al` files and `app.json` diagnostics. The tool automatically opens and focuses the specified `app.json` file before building (required by AL extension), then executes the `al.package` command and collects all compilation diagnostics. This detailed error information enables LLMs to diagnose and fix compilation issues effectively, making it particularly valuable for translation workflows where build validation is critical. Note: This tool may be deprecated once the `al_build` tool is improved to return proper error information.
