...
- Changed:
  - The `openFile` Language Model Tool now supports non-file URI schemes (e.g. `al-preview://`). When a URI with a non-file scheme is provided, the filesystem existence check is skipped and the URI is passed directly to the VS Code document provider. This enables the tool to open virtual documents registered via `TextDocumentContentProvider`, such as AL preview files generated by the AL extension.
  - Updated dependencies (combined Dependabot updates): `adm-zip` (0.5→0.6), `@modelcontextprotocol/sdk` (1.26→1.30), `@xmldom/xmldom` (0.8.13→0.8.15), `js-yaml` (4.1→4.3), plus transitive security bumps for `body-parser`, `ip-address`, `postcss`, `fast-uri`, `nanoid`, `@humanfs/node`, `qs`, `browserslist`, `baseline-browser-mapping`, and `hono`.
- Fixes:
  - Fixed an issue where `BinaryReader` could read `.app` package files incorrectly when `fs.readFileSync` returned a pooled Buffer (a view into a larger `ArrayBuffer` with a non-zero `byteOffset`). The reader now resolves the absolute position within the underlying `ArrayBuffer`, so app files are parsed correctly regardless of buffer pooling. This manifested as `"... is not a valid app file"` errors in the VS Code test host on newer VS Code versions.
