...
- Added:
  ...
  - Added **dedicated review workflow** for translation quality control through new `reviewXlfFiles` prompt file that works with the agent. This enables focused review sessions for translations in "needs-review-translation" state with clean markdown table presentation (default: 10 items per batch), flexible user input patterns (`1,3,5` to accept, `2:Custom Text` to modify, `skip 4,6` to defer), and proper state management. The agent analyzes alternative translations and applies glossary-based suggestions.
