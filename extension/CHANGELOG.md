...
- Added:
  - Added support for reading "ready-to-run" app files (BC 28+). When `AppPackage.fromFile()` encounters a ready-to-run wrapper (a NAVX container whose inner ZIP contains `readytorunappmanifest.json`), it transparently reads the embedded inner `.app` file in-memory without writing to disk. The manifest and symbol reference are extracted from the inner app, so the caller receives a fully populated `AppPackage` as if the wrapper did not exist.
- Changed:
