...
- Changed:
  - **Deprecated DTS functionality**: All Dynamics 365 Translation Service (DTS) related functionality is now deprecated and will be removed in the beginning of 2026. Microsoft has announced that DTS will be retired. When using DTS-related commands (`NAB: Format current XLF file for DTS`, `NAB: Open DTS`, `NAB: Import DTS Translations`), a deprecation warning is shown with options to continue, dismiss the warning permanently, or learn more. Telemetry tracking has been added to monitor deprecated feature usage. The following settings already show deprecation messages: `NAB.UseDTS`, `NAB.DTS ProjectId`, `NAB.Set DTS Exact Match To State`, `NAB.useDictionaryInDTSImport`.
- Fixes:
