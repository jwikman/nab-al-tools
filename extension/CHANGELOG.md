...
- New Features:
  ...
  - `NAB: Create AL Project from Template (preview)` supports specifing `postConversionTasks` in the `al.template.json` file. The commands specified will we executed as the last step in in the conversion process or after reload of VS Code.
  - `NAB: Report Issue` a shortcut to initate an issue with VS Code's Issue Reporter.
- Fixes:
