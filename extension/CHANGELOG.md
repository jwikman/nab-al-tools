...
- Added:
  ...
  - Added `outputFormat` parameter to Language Model Tools (ChatTools only, not MCP). Supports `"json"` and `"tsv"` output formats. Glossary defaults to TSV; translation tools default to JSON. TSV is not supported for tools with nested array fields (`getTranslatedTextsMap`, `getTranslatedTextsByState`, `getTextsByKeyword`).
  - Added `returnAsFile` parameter to `getGlossaryTerms` and `getTranslatedTextsMap` Language Model Tools. When `true`, writes the result to a file and returns the file path instead of inline content. Useful for large results and for providing stable context to translation subagents.
- Changed:
  ...
  - **Translation workflow restructured for subagent architecture**: The NAB-XLF-Translator agent now supports self-looping translation with fresh context per chunk. The orchestrator pre-loads glossary and translated texts to files, then spawns subagents that loop `getTextsToTranslate → translate → setTranslations` until complete.
