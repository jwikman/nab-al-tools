...
- Added:
  ...
  - Added `sampling` parameter to `getTranslatedTextsMap` Language Model Tool (ChatTools only, not MCP). When set to `"even"`, selects evenly-spaced entries across the full translation map instead of returning the first N entries. This provides better vocabulary coverage when loading reference translations for AI-assisted translation workflows. When `sampling="even"`, the `offset` parameter is ignored.
...
- Changed:
  ...
  - **Translation tool output envelope (ChatTools only)**: The `getTextsToTranslate`, `getTranslatedTextsMap`, `getTranslatedTextsByState`, and `getTextsByKeyword` Language Model Tools (ChatTools) now wrap output in an envelope object with `sourceLanguage` at the top level. Per-item `sourceLanguage` is removed from serialized output. For `getTextsToTranslate`: `{ sourceLanguage, totalUntranslatedCount, returnedCount, texts: [...] }`. For other tools: `{ sourceLanguage, items: [...] }`. MCP server tools are not affected.
  - **Translation workflow restructured for subagent architecture**: The NAB-XLF-Translator agent now supports self-looping translation with fresh context per chunk. Each translator subagent fetches glossary and translated texts map directly at session start, then loops `getTextsToTranslate → translate → saveTranslatedTexts` until complete.
  ...
  - Note: The glossary TSV and sourceLanguage envelope changes are breaking changes for ChatTools consumers. The `outputFormat` parameter provides backward compatibility — consumers can request `outputFormat: "json"` to get the previous JSON format for glossary. MCP server tools are not affected by these changes.
- Fixes:
