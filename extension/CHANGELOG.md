...
- Added:
  ...
  - Added new command line parameters to `RefreshXLF.js` for improved CI/CD integration, particularly GitHub Actions workflows:
    - `--github-message`: Formats output as GitHub Actions workflow commands (::warning:: and ::error::) for better integration with GitHub Actions. Output is grouped by file with a header line followed by detail lines prefixed with " - ".
